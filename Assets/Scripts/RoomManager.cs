...
public class RoomManager : MonoBehaviourPunCallbacks
{
    ...
    public void HandleTeamSelection()
    {
        ...
        if (selectedTeam == 0) // N?u ??i t?n c�ng ???c ch?n
        {
            ...
            spawnPoint = attackSpawnPoints[Random.Range(0, attackSpawnPoints.Length)]; // Ch?n ng?u nhi�n ?i?m spawn cho ??i t?n c�ng
            teamPrefab = attackTeamPrefab; // Nh�n v?t cho ??i t?n c�ng
        }
        else // N?u ??i ph�ng th? ???c ch?n
        {
            Debug.Log("??i ph�ng th? ???c ch?n.");
            spawnPoint = defenseSpawnPoints[Random.Range(0, defenseSpawnPoints.Length)]; // Ch?n ng?u nhi�n ?i?m spawn cho ??i ph�ng th?
